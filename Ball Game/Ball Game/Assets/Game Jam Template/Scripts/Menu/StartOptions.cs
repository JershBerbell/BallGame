...
public class StartOptions : MonoBehaviour
{
    ...
    [HideInInspector] public bool inGameScene = false;                  //If true, game scene has loaded and it's time to fade back in
    ...
    public IEnumerator LoadDelayedAsync()
    {
        //Load the selected scene, by scene index number in build settings
        AsyncOperation loadSceneAsync = SceneManager.LoadSceneAsync("Level1");
        while (!loadSceneAsync.isDone)
        {
            //uncomment this line to see the progress of your scene loading in the console
            //print(loadSceneAsync.progress);
            yield return null;
        }
        //these lines will not run until the game scene is loaded
        inGameScene = true;
    ...
    public IEnumerator FadeCanvasGroupAlpha(float startAlpha, float endAlpha, CanvasGroup canvasGroupToFadeAlpha)
    {
        ...
        {
            Debug.Log("Coroutine done. Game started in new scene! Put your game starting stuff here.");
        }
    }
